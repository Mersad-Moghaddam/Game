...
export class FX{
  constructor(){this.p=[];this.decals=[];this.flashes=[];this.after=[];this.corpses=[];this.rings=[];this.casings=[];this.bloodEnabled=true;this.quality=1;}
  ...
  casing(x,y,a){if(this.casings.length>140)this.casings.shift();const side=Math.random()<.5?1:-1,ea=a+Math.PI/2*side;this.casings.push({x,y,vx:Math.cos(a)*rand(20,60)+Math.cos(ea)*rand(70,140),vy:Math.sin(a)*rand(20,60)+Math.sin(ea)*rand(70,140),a:rand(0,6.283),spin:rand(-16,16),life:rand(.8,1.4)});}
  smoke(x,y,a){const n=2+((Math.random()*2)|0);for(let i=0;i<n;i++)this.p.push({x,y,vx:Math.cos(a)*rand(25,70)+rand(-18,18),vy:Math.sin(a)*rand(25,70)+rand(-18,18),life:rand(.3,.6),max:.6,size:rand(2,4.5),color:'#8a8a92',glow:false});}
  update(dt){for(const p of this.p){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vx*=Math.pow(.1,dt);p.vy*=Math.pow(.1,dt);p.life-=dt;}this.p=this.p.filter(p=>p.life>0);for(const c of this.casings){c.x+=c.vx*dt;c.y+=c.vy*dt;c.vx*=Math.pow(.02,dt);c.vy*=Math.pow(.02,dt);c.a+=c.spin*dt;c.life-=dt;}this.casings=this.casings.filter(c=>c.life>0);for(const f of this.flashes)f.life-=dt;this.flashes=this.flashes.filter(f=>f.life>0);for(const a of this.after)a.life-=dt;this.after=this.after.filter(a=>a.life>0);for(const r of this.rings)r.life-=dt;this.rings=this.rings.filter(r=>r.life>0);}
  ...
  draw(ctx){ctx.fillStyle='#c9a24a';for(const c of this.casings){if(c.life<.2)continue;ctx.save();ctx.translate(c.x,c.y);ctx.rotate(c.a);ctx.fillRect(-1.6,-1,3.2,2);ctx.restore();}for(const a of this.after){ctx.globalAlpha=a.life/.18*.22;ctx.save();ctx.translate(a.x,a.y);ctx.rotate(a.a);ctx.fillStyle='#12e0ff';ctx.fillRect(-10,-7,20,14);ctx.restore();}for(const p of this.p){ctx.globalAlpha=Math.max(0,p.life/p.max);ctx.fillStyle=p.color;ctx.fillRect(p.x-p.size/2,p.y-p.size/2,p.size,p.size);}ctx.globalAlpha=1;}
  drawGlow(ctx){
