...
export class FX{
  ...
  arterial(x,y,angle){
    if(!this.bloodEnabled)return;
    for(let i=0;i<16;i++){const a=angle+rand(-.5,.5),s=rand(150,430);this.p.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:rand(.4,.9),max:.9,size:rand(2,5),color:'#ff0a3c',glow:false});}
    for(let i=0;i<5;i++){const a=angle+rand(-.4,.4),s=rand(60,150);this.p.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:rand(.5,1),max:1,size:rand(4,8),color:'#8e0f22',glow:false});}
    this.pool(x,y,3);
  }
